...
const OwnersDashboard: React.FC = () => {
  ...
  const occupancyData = useMemo(() => [
    ...
    { name: 'Vacant', value: stats.totalUnits - stats.occupiedUnits, color: '#F59E0B' },
  ], [stats]);
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">Manage your real estate investments</p>
            </div>
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-4">
              <div className="flex items-center">
                ...
                <p className="text-red-800">
                  You have {stats.overduePayments} overdue payments requiring attention.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-200">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-3xl font-bold text-gray-900 mt-2">{stats.avgOccupancyRate}%</p>
                  <div className="flex items-center mt-2 text-sm">
                    <TrendingUp className="h-4 w-4 text-green-500 mr-1" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Occupancy Overview */}
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900 mb-6">Unit Occupancy Overview</h3>
              <div className="flex items-center justify-center">
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    ...
                    <Tooltip formatter={(value: number) => [`${value} units`, 'Count']} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
              {properties.map((property) => {
                ...
                return (
                  <div key={property.id} className="border border-gray-200 rounded-xl hover:shadow-md transition-shadow duration-200">
                    <div className="relative">
                      <img
                        src={property.image}
                        alt={property.name}
                    ...
                    <div className="p-6">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-600">Units Occupied</span>
                          <span className="font-medium">{property.occupiedUnits}/{property.units}</span>
                        </div>
                        <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-medium text-blue-600">{formatCurrency(property.monthlyRevenue)}</span>
                        </div>
                      </div>
                ...
                <div key={transaction.id} className="p-6 hover:bg-gray-50 transition-colors duration-200">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      <div className={`p-3 rounded-lg ${transaction.type === 'rent' ? 'bg-green-100' :
                        transaction.type === 'maintenance' ? 'bg-orange-100' :
                          transaction.type === 'deposit' ? 'bg-blue-100' :
                            'bg-gray-100'
                        }`}>
                        {transaction.type === 'rent' && <DollarSign className="h-5 w-5 text-green-600" />}
                        {transaction.type === 'maintenance' && <Wrench className="h-5 w-5 text-orange-600" />}
                        {transaction.type === 'deposit' && <CreditCard className="h-5 w-5 text-blue-600" />}
                        {transaction.type === 'utility' && <Zap className="h-5 w-5 text-purple-600" />}
                      </div>
                      <div>
                    ...
                    <div className="text-right">
                      ...
                      <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium border ${getStatusColor(transaction.status)} mt-1`}>
                        {transaction.status}
                      </span>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
            <div className="p-6 border-b border-gray-100">
              ...
              <p className="text-sm text-gray-600 mt-1">Common tasks for property management</p>
            </div>
