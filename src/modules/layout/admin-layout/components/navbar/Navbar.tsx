...
const PropertifyNavbar: React.FC = () => {
  ...
  const getPriorityBadge = useCallback(
    (priority: NotificationItem["priority"]) => {
      const priorityColors = {
        ...
        medium:
          "bg-yellow-500 text-white text-xs px-2 py-1 rounded-full font-medium",
        low: "bg-green-500 text-white text-xs px-2 py-1 rounded-full font-medium",
      };
      return (
        priorityColors[priority] ||
        "bg-gray-500 text-white text-xs px-2 py-1 rounded-full font-medium"
      );
    },
    []
  );
  ...
  return (
    <>
      <nav
        className={`bg-white shadow-lg border-b border-gray-100 fixed w-full right-0 left-0 top-0 z-50 transition-all duration-300 ${
          isScrolled
            ? "shadow-2xl backdrop-blur-md bg-white/95 border-gray-200"
            : ""
        }`}
      >
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center justify-items-start">
              <div className="flex-shrink-0 flex items-center group cursor-pointer">
                ...
                <div className="mr-4 ml-[-18px] rounded-[2px] bg-blue-500">
                  ...
                  >
                    {isCollapsed ? (
                      <PanelLeftOpen className="h-5 w-5" color="#fff" />
                    ) : (
                      <PanelLeftClose className="h-5 w-5" color="#fff" />
                    )}
                  </button>
                </div>
                      ...
                      {item.dropdown ? (
                        <div className="group">
                          ...
                          >
                            ...
                            <ChevronDown
                              className={`h-3 w-3 ml-1.5 transition-transform duration-200 ${
                          ...
                          {/* Dropdown Menu */}
                          <div
                            className={`absolute left-0 mt-1 w-80 bg-white rounded-xl shadow-2xl border border-gray-100 transition-all duration-200 z-50 ${
                              activeDropdown === item.id
                            ...
                            <div className="p-3">
                              <div className="flex items-center justify-between px-3 py-2 mb-2">
                                ...
                                <span className="text-xs text-gray-500">
                                  {item.dropdown?.length || 0} options
                                </span>
                              </div>
                              <div className="space-y-1">
                                {item.dropdown?.map((dropdownItem: any) => (
                                  <button
                                    ...
                                    onClick={() =>
                                      handleNavigate(dropdownItem.href)
                                    }
                        ...
                        <button
                          ...
                          className="flex items-center text-gray-700 hover:text-blue-600 hover:bg-blue-50 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group"
                        >
                          <item.icon className="h-4 w-4 mr-2.5 group-hover:scale-110 transition-transform duration-200" />
                          {item.label}
                          {item.badge && (
            ...
            <div className="flex items-center space-x-3">
                    ...
                    <div className="max-h-96 overflow-y-auto">
                      {notifications.map((notification) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`p-2.5 rounded-lg mr-3 border ${getNotificationColor(
                                notification.type,
              ...
              {/* User Menu */}
              <div className="relative group dropdown-container">
                <button className="flex items-center space-x-3 bg-blue-100 hover:bg-gray-100  px-3  transition-all duration-200 mr-[-1.8rem]">
                  <div className="relative">
                  ...
                  <div className="hidden md:block text-left">
                    ...
                    <div className="flex items-center space-x-2">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-lg font-medium ${getRoleBadgeColor(
                          user.role
                        )}`}
                      >
                        Property Owner
