...
import { getAllTenants } from "../features/tenant.slice";

import Layout from "@/modules/layout/admin-layout/Layout";
import { getStatusBadge } from "../utils/utils";
import TenantsList from "../components/TenantsList";
import Header from "../components/Header";
import type { TenantApplication, User } from "../types/type";
import type { ActionType, Estate } from "../types/tenants-slice";
...
const AllTenants: React.FC = () => {
  ...
  // Fetch tenants from Redux on component mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Dispatch the Redux action to get tenants
        await dispatch(getAllTenants({ page: 1, limit: 10 })).unwrap();
        
        // You can keep your existing API calls for applications and estates
        const [applicationsData, estatesData] = await Promise.all([
          // getTenantApplications(), // Uncomment if you have this
          Promise.resolve([]), // Placeholder for applications
          Promise.resolve([])  // Placeholder for estates
        ]);
  ...
  // Handle action from TenantsList
  const handleAction = (action: string, tenant: any): void => {
    const actionType = action as ActionType;
    
    if (actionType === 'view' || actionType === 'edit') {
      // Handle view/edit actions directly
      console.log(`${action} tenant:`, tenant);
      // Implement your view/edit logic here
      if (actionType === 'view') {
        // Navigate to tenant details or open view modal
      } else if (actionType === 'edit') {
        // Navigate to edit page or open edit modal
      }
    } else {
      // For suspend/activate/approve/reject actions, show the dialog
    ...
    try {
      if (actionType === "approve" || actionType === "reject") {
              ...
              app.id === selectedApplication.id
                ? {
                    ...
                    applicationStatus:
                      actionType === "approve" ? "approved" : "rejected",
                    rejectionReason:
                      actionType === "reject" ? actionReason : undefined,
                  }
                : app
            )
          );
        ...
        if (selectedTenant) {
          ...
          // Note: You would need to dispatch a Redux action here to update tenant status
          // For now, we'll just close the dialog
          console.log(`Updating tenant ${selectedTenant.id} status to ${newStatus}`);
        }
      }
    ...
    <Layout>
      <div className="p-6 bg-gray-50 min-h-screen">
        ...
        <Header
          ...
          setEstateFilter={setEstateFilter}
        />
        ...
        >
          ...
          <TabsContent value="tenants" className="space-y-6">
            ...
            {/* Tenants List - Now using Redux tenants */}
            <TenantsList
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-gray-500">
                              ...
                              • KRA PIN: {application.kraPin} • Emergency:{" "}
                          ...
                          <div className="flex space-x-2">
                            <Button
                              ...
                              onClick={() =>
                                handleAction("approve", undefined, application)
                              }
                            >
                            ...
                            <Button
                              ...
                              onClick={() =>
                                handleAction("reject", undefined, application)
                              }
                            >
          ...
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {actionType === "suspend" &&
                  "This will suspend the tenant account. Please provide a reason."}
                {actionType === "activate" &&
                  "This will activate the tenant account."}
              </DialogDescription>
            </DialogHeader>
